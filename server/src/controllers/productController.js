...
const findDuplicateProductName = async (name, excludedId = null) => {
...
module.exports = {
  ...
  createProduct: async (req, res, next) => {
    try {
      ...
      if (await findDuplicateProductName(productData.name)) {
        res.status(409);
  ...
  updateProduct: async (req, res, next) => {
    try {
      ...
      const { productId: _productId, ...productData } = req.body;
      if (await findDuplicateProductName(productData.name, req.params.id)) {
        res.status(409);
        throw new Error("A product with this name already exists.");
      }
